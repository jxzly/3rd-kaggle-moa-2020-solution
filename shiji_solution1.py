    ...
    for fold, (trn_ind, val_ind) in enumerate(MultilabelStratifiedKFold(n_splits = folds, shuffle=True, random_state=seed)\
        ...
        for epoch in range(EPOCHS1):
            ...
            if valid_loss < best_valid_metric:
                torch.save(model.state_dict(),'./model_dnn_final_fold%s'%fold+'_'+str(seed)+'.ckpt')
                not_improve_epochs = 0
        ...
        state_dict = torch.load('./model_dnn_final_fold%s'%fold+'_'+str(seed)+'.ckpt', torch.device("cuda" if torch.cuda.is_available() else "cpu") )
